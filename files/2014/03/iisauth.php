...
function iisauth_is_lan_user() {
	...
	return (substr($remoteAddress, 0, 8) === '192.168.' || substr($remoteAddress, 0, 3) === '10.' || substr($remoteAddress, 0, 3) === '172.16.');
}
...
{ 
    ...
    if ((preg_match('/MSIE/i', $userAgent) && preg_match('/Win/i', $userAgent)) || preg_match('/Trident\/7.0; rv:11.0/', $userAgent))
    { 
        return true;
    }
    return false; 
